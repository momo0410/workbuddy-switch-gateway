...
//	              state 落系统临时目录的 wb2api-login-state.json，stdout 打印授权 URL
...
// stateFile 暂存 url 与 poll 两次调用之间的 state。
//
// 用系统临时目录而非写死 "/tmp"：Windows 上没有 /tmp，写死会落到当前盘根目录下
// 的 \tmp\，且可能因权限失败。os.TempDir() 会读 TMPDIR/TEMP/TMP，跨平台一致。
